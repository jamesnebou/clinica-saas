...
5. Criar um System User Admin, registrar seu ID e gerar o token server-side com `business_management`, `whatsapp_business_management` e `whatsapp_business_messaging`.
...
9. Configurar todas as variáveis `META_*` de `.env.example` em Preview e Production na Vercel, incluindo `META_SYSTEM_USER_ID` e um `META_PHONE_REGISTRATION_SECRET` aleatório com pelo menos 32 caracteres.
...
O Config ID, seus escopos e os domínios permitidos são configuração externa do Meta Dashboard. Consulte também `docs/whatsapp-meta-app-review.md` antes de homologar ou gravar o screencast.
