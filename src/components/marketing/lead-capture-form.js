...
import { marketingWhatsAppHref } from "@/lib/marketing/commercial-links.mjs";

export function LeadCaptureForm({
  segment = "geral",
  variant = "current",
  clinicPlaceholder = "Ex.: Clínica Bella Skin",
  selectedPlan = "nao_sei",
  ...
  function trackFormStart() {
    ...
    trackMarketingEvent("lead_form_start", { segment, plan });
  }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      try { setPlan(window.localStorage.getItem("nexawi_selected_plan") || selectedPlan); } catch {}
    }, 0);
  ...
  return (
    <section id="contato" className="marketing-section scroll-mt-24 px-5 py-20 sm:px-8 lg:px-10">
      <div className={variant === "premium-v2" ? "premium-lead-layout" : "mx-auto grid max-w-7xl overflow-hidden rounded-lg bg-[#1c1c1c] text-white shadow-[0_34px_100px_rgba(28,28,28,0.24)] lg:grid-cols-[0.8fr_1.2fr]"}>
        <div className={variant === "premium-v2" ? "premium-lead-intro" : "bg-[#202020] p-7 lg:p-10"}>
        ...
        <form onSubmit={submit} onFocus={trackFormStart} className={variant === "premium-v2" ? "premium-lead-form" : "grid gap-4 bg-white/[0.07] p-7 backdrop-blur lg:p-10 sm:grid-cols-2"}>
          <PublicFormGuard />
          ...
          <label className="text-sm font-bold text-white/80">Nome da clínica
            <input name="clinic_name" maxLength={120} placeholder={clinicPlaceholder} className="mt-2 h-12 w-full rounded-md border border-white/12 bg-black/25 px-4 text-white outline-none placeholder:text-white/30 focus:border-orange-400" />
          </label>
          <label className="text-sm font-bold text-white/80">Quantidade de profissionais
            <select name="professionals_count" required defaultValue="" className="mt-2 h-12 w-full rounded-md border border-white/12 bg-[#202020] px-4 text-white outline-none focus:border-orange-400">
          ...
          <label className="premium-contact-consent flex cursor-pointer items-start gap-3 rounded-md border border-white/15 bg-white/[0.04] px-4 py-3 text-xs leading-5 text-white/78 sm:col-span-2">
            <input name="contact_consent" type="checkbox" required className="mt-0.5 h-4 w-4 shrink-0 accent-orange-400" /> Autorizo o contato da NexaWi sobre esta solicitação, conforme a Política de Privacidade.
          </label>
          ...
          <a href={marketingWhatsAppHref(segment)} target="_blank" rel="noreferrer" onClick={() => trackMarketingEvent("whatsapp_click", { location: "lead_form", segment })} className="text-center text-xs font-bold text-white/55 underline underline-offset-4 sm:col-span-2">Prefere não preencher? Fale direto pelo WhatsApp.</a>
        </form>
      </div>
    </section>
  );
