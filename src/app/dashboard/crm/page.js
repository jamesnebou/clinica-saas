...
export default async function CrmPage() {
  ...
  const workspace = await getCrmWorkspace(activeClinic.id);
  return <main className="min-w-0 w-full px-4 py-8 sm:px-6 lg:px-8">
    <section className="mx-auto w-full min-w-0 max-w-[1680px]">
      ...
      {!workspace.available ? <div className="mt-6"><Notice type="warning" title="CRM 2.0 aguardando ativação">Aplique as migrations `20260828101000`, `20260828102000` e `20260828103000` no Supabase, nessa ordem.</Notice></div> : <CrmBoard workspace={workspace} />}
    </section>
  </main>;
}
