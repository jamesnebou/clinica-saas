  ...
  return (
    <main className="px-5 py-8 sm:px-8 lg:px-10">
      <section className="mx-auto max-w-7xl">
        <PageHeader eyebrow="Dashboard" title="Operação da clínica" description={`Visão executiva de ${activeClinic.nome}: agenda, clientes, equipe, financeiro e status comercial.`} />
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-[1fr_360px]">
            ...
            {cards.map((card) => {
              ...
              return (
                <article key={card.label} className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center justify-between"><span className="text-sm font-medium text-neutral-500">{card.label}</span><Icon size={20} className="text-emerald-600" /></div>
          ...
          <aside className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-2"><ShieldCheck size={20} className="text-emerald-700" /><h2 className="font-semibold">Plano e acesso</h2></div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
          <section className="rounded-lg border border-neutral-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><h2 className="text-lg font-semibold">Próximos agendamentos</h2><Link href="/dashboard/agenda" className="text-sm font-semibold text-emerald-700">Abrir agenda</Link></div>
            <div className="mt-4 space-y-3">
              {proximos.length === 0 ? (
                <EmptyState title="Agenda livre nos próximos horários" description="Cadastre um atendimento para demonstrar status, WhatsApp rápido, faturamento previsto e filtro por profissional." action={<Link href="/dashboard/agenda" className="inline-flex h-10 items-center rounded-lg bg-neutral-950 px-4 text-sm font-semibold text-white">Criar agendamento</Link>} />
              ) : proximos.map((item) => (
                <div key={item.id} className="rounded-lg border border-neutral-200 p-4">
                  <div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{item.clientes?.nome || "Cliente não informado"}</p><p className="mt-1 text-sm text-neutral-600">{item.procedimentos?.nome || "Procedimento não informado"}</p></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold uppercase text-emerald-700">{item.status}</span></div>
