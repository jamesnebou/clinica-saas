...
export default async function AutomationDetailPage({ params, searchParams }) {
  ...
  return <main className="min-w-0 w-full overflow-x-hidden px-4 py-6 sm:px-6 sm:py-8 lg:px-8 xl:px-10">
    ...
    <nav aria-label="Seções da automação" className="mt-6 flex max-w-full gap-2 overflow-x-auto rounded-lg border border-neutral-200 bg-white/75 p-2">
      {[['overview','Visão geral'],['builder','Builder'],['runs','Execuções'],['settings','Configurações']].map(([anchor, label]) => <a key={anchor} href={`#${anchor}`} className="shrink-0 rounded-lg px-4 py-2 text-sm font-bold text-neutral-600 hover:bg-neutral-950 hover:text-white">{label}</a>)}
    </nav>

    <section id="overview" className="mt-6 scroll-mt-6 flex flex-wrap items-center justify-between gap-4 rounded-lg border border-neutral-200 bg-white/75 p-4 sm:p-5">
      ...
      <div className="flex w-full flex-wrap gap-2 sm:w-auto">{automation.current_version_id ? <form action={setAutomationStatusAction} className="w-full sm:w-auto"><input type="hidden" name="automation_id" value={automation.id}/><input type="hidden" name="status" value={automation.status === "active" ? "paused" : "active"}/><button className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-neutral-200 bg-white px-4 py-2 text-sm font-bold sm:w-auto">{automation.status === "active" ? <CirclePause size={16}/> : <CirclePlay size={16}/>} {automation.status === "active" ? "Pausar" : "Reativar"}</button></form> : null}<form action={publishAutomationAction} className="w-full sm:w-auto"><input type="hidden" name="automation_id" value={automation.id}/><button className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-neutral-950 px-4 py-2 text-sm font-bold text-white sm:w-auto"><ShieldCheck size={16}/> Validar e publicar</button></form></div>
    </section>
