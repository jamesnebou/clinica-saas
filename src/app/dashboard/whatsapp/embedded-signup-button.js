...
export function EmbeddedSignupButton() {
  ...
  async function connect() {
    ...
    try {
      const start = await fetch("/api/whatsapp/embedded-signup/start", { method: "POST" }).then(async (response) => ({
        ok: response.ok,
      ...
      const finish = await fetch("/api/whatsapp/embedded-signup/callback", {
        ...
        body: JSON.stringify({ state: start.data.state, code, wabaId, phoneNumberId }),
      }).then(async (finishResponse) => ({
        ok: finishResponse.ok,
        data: await finishResponse.json(),
      }));
  ...
  return (
    <div>
      <button
        ...
        className="inline-flex h-11 items-center gap-2 rounded-lg bg-[#25D366] px-5 text-sm font-black text-white shadow-lg transition hover:-translate-y-0.5 disabled:opacity-60"
      >
        {status === "loading" ? <LoaderCircle className="animate-spin" size={18} /> : <Link2 size={18} />}
        Conectar WhatsApp
      </button>
      {message ? <p className={`mt-2 text-sm ${status === "error" ? "text-red-700" : "text-emerald-700"}`}>{message}</p> : null}
    </div>
  );
}
