...
export function buildDemoDataset({ clinicId, userId, now = new Date() }) {
  ...
  table.produtos_clinica = [
    { id: id("product:serum"), clinica_id: clinicId, nome: "Sérum antioxidante", sku: "DEMO-SERUM", categoria: "Home care", descricao: "Sérum facial demonstrativo.", custo: 62, preco: 149, estoque_atual: 18, estoque_reservado: 0, estoque_minimo: 5, publicado_site: true, ativo: true },
    { id: id("product:protetor"), clinica_id: clinicId, nome: "Protetor solar premium", sku: "DEMO-FPS", categoria: "Home care", descricao: "Proteção diária para protocolos faciais.", custo: 48, preco: 119, estoque_atual: 12, estoque_reservado: 0, estoque_minimo: 4, publicado_site: true, ativo: true },
    ...
    { id: id("product:sabonete"), clinica_id: clinicId, nome: "Sabonete facial suave", sku: "DEMO-SAB", categoria: "Home care", descricao: "Limpeza diária para peles sensíveis.", custo: 29, preco: 79, estoque_atual: 24, estoque_reservado: 0, estoque_minimo: 8, publicado_site: true, ativo: true },
    { id: id("product:hidratante"), clinica_id: clinicId, nome: "Hidratante reparador", sku: "DEMO-HIDRA", categoria: "Home care", descricao: "Hidratação intensiva pós-procedimento.", custo: 44, preco: 109, estoque_atual: 16, estoque_reservado: 0, estoque_minimo: 5, publicado_site: true, ativo: true },
    { id: id("product:mascara"), clinica_id: clinicId, nome: "Máscara calmante", sku: "DEMO-MASC", categoria: "Home care", descricao: "Máscara calmante para rotina semanal.", custo: 38, preco: 99, estoque_atual: 4, estoque_reservado: 0, estoque_minimo: 6, publicado_site: true, ativo: true },
    { id: id("product:oleo"), clinica_id: clinicId, nome: "Óleo corporal nutritivo", sku: "DEMO-OLEO", categoria: "Corporal", descricao: "Cuidado corporal para uso diário.", custo: 36, preco: 89, estoque_atual: 21, estoque_reservado: 0, estoque_minimo: 6, publicado_site: true, ativo: true },
    { id: id("product:necessaire"), clinica_id: clinicId, nome: "Nécessaire de autocuidado", sku: "DEMO-NEC", categoria: "Kits", descricao: "Kit presenteável da clínica.", custo: 52, preco: 139, estoque_atual: 9, estoque_reservado: 0, estoque_minimo: 4, publicado_site: true, ativo: true },
  ];
  ...
  table.pedidos_clinica = [
    { id: id("order:paid"), clinica_id: clinicId, cliente_id: client.mariana, status: "concluido", pagamento_status: "pago", entrega_tipo: "retirada", nome_cliente: "Mariana Costa", telefone_cliente: "77988887777", email_cliente: "mariana.demo@nexawi.com.br", subtotal: 268, desconto: 0, frete: 0, total: 268, forma_pagamento: "PIX", pago_em: localTimestamp(now, -3, 13), origem: { source: "site_demo" }, observacoes: "Pedido demonstrativo pago.", created_at: localTimestamp(now, -3, 12) },
    { id: id("order:pending"), clinica_id: clinicId, cliente_id: client.bianca, cupom_id: id("coupon:BEMVINDA"), status: "aguardando_pagamento", pagamento_status: "pendente", entrega_tipo: "retirada", nome_cliente: "Bianca Teixeira", telefone_cliente: "77910101010", email_cliente: "bianca.demo@nexawi.com.br", subtotal: 219, desconto: 21.9, frete: 0, total: 197.1, cupom_codigo: "BEMVINDA", forma_pagamento: "PIX", expiracao_reserva_em: localTimestamp(now, 1, 18), origem: { source: "instagram", campaign: "home_care_demo" }, observacoes: "Pedido aguardando pagamento.", created_at: localTimestamp(now, 0, 10) },
  ];
  table.pedido_itens_clinica = [
    { id: id("order-item:paid:serum"), clinica_id: clinicId, pedido_id: id("order:paid"), produto_id: id("product:serum"), nome_produto: "Sérum antioxidante", sku: "DEMO-SERUM", quantidade: 1, valor_unitario: 149, desconto: 0, total: 149, created_at: localTimestamp(now, -3, 12) },
    { id: id("order-item:paid:fps"), clinica_id: clinicId, pedido_id: id("order:paid"), produto_id: id("product:protetor"), nome_produto: "Protetor solar premium", sku: "DEMO-FPS", quantidade: 1, valor_unitario: 119, desconto: 0, total: 119, created_at: localTimestamp(now, -3, 12) },
    { id: id("order-item:pending:kit"), clinica_id: clinicId, pedido_id: id("order:pending"), produto_id: id("product:kit"), nome_produto: "Kit pós-procedimento", sku: "DEMO-KIT", quantidade: 1, valor_unitario: 219, desconto: 21.9, total: 197.1, created_at: localTimestamp(now, 0, 10) },
  ];
  table.pagamentos_loja_clinica = [
    { id: id("shop-payment:paid"), clinica_id: clinicId, cliente_id: client.mariana, pedido_id: id("order:paid"), valor: 268, forma: "pix", status: "pago", provedor: "demo", provedor_pagamento_id: `demo-paid-${id("order:paid")}`, pago_em: localTimestamp(now, -3, 13), observacoes: "Pagamento fictício.", created_at: localTimestamp(now, -3, 12) },
    { id: id("shop-payment:pending"), clinica_id: clinicId, cliente_id: client.bianca, pedido_id: id("order:pending"), valor: 197.1, forma: "pix", status: "pendente", provedor: "demo", provedor_pagamento_id: `demo-pending-${id("order:pending")}`, vencimento_em: localTimestamp(now, 1, 18), observacoes: "Cobrança fictícia.", created_at: localTimestamp(now, 0, 10) },
  ];
  ...
  table.estoque_reservas_clinica.push({ id: id("stock-reservation:pending"), clinica_id: clinicId, pedido_id: id("order:pending"), produto_id: id("product:kit"), quantidade: 1, status: "ativa", expira_em: localTimestamp(now, 1, 18), created_at: localTimestamp(now, 0, 10) });
  table.estoque_movimentos_clinica.push({ id: id("stock-movement:pending"), clinica_id: clinicId, produto_id: id("product:kit"), pedido_id: id("order:pending"), tipo: "reserva", quantidade: 0, estoque_anterior: 7, estoque_posterior: 7, observacoes: "Reserva temporária do pedido pendente.", created_by: userId, created_at: localTimestamp(now, 0, 10) });
