...
declare
  ...
  v_solicitacoes_preservadas integer := 0;
...
begin
  ...
  if exists (
    select 1
    from public.site_agendamentos_publicos s
    where s.clinica_id = p_clinica_id
      and s.agendamento_id = p_agendamento_id
      and (
        s.pagamento_status = 'pago'
        or s.asaas_payment_id is not null
        or s.pagamento_external_id is not null
      )
  ) then
    raise exception 'Agendamentos com cobrança online não podem ser excluídos. Cancele o agendamento para preservar o pagamento.' using errcode = 'P0001';
  end if;
  ...
  get diagnostics v_solicitacoes_preservadas = row_count;
  ...
  return jsonb_build_object(
    ...
    'solicitacoes_site_preservadas', v_solicitacoes_preservadas,
